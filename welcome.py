...
for line in signDataMentors:
    ...
    if re.match(r'^\*\s*?\[\[Участни(?:к|ца):(.*?)\]\]\s*?\|(.*)', line):
        ...
        if r.group(2) not in preSignList:
            preDefaultPhrase = defaultPhrase.replace("$", "участнику") if (r.group(1) == "Участник") else defaultPhrase.replace("$", "участнице")
            preSignList[r.group(2)] = {"gender": r.group(1), "phrase": preDefaultPhrase + "[[ОУ:" + r.group(2) + "|"
                                       + r.group(2) + "]] — ~~~~~"}
...
# timestamp = ((datetime.datetime.utcnow() - datetime.timedelta(minutes = minutes)).strftime("%Y-%m-%d %H:%M:%S")
payload = {'action': 'query', 'format': 'json', 'list': 'recentchanges',
'rcprop': 'user|timestamp|ids', 'rcshow': '!bot|!anon', 'rctype': 'new|edit', 'rcend': timestamp, 'rclimit': 5000}
...
for user in users:
    if user['user'] not in usersCheck and user["rcid"] != rcid:
