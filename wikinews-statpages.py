...
with open("pyBot/wikinews/service/statpages.txt", "r", encoding="utf-8") as f:
    lines = f.readlines()
    f.close()
...
def handler(members, timestamp, pagename):
    ...
    for member in members:
        if str(member["title"]) != pagename and not str(member["title"]).startswith("Лента новостей "):
            params = {
                ...
                "summary": "Создание страницы статистики", "bot": 1, "token": token
            }
            requests.post(url=API_url, data=params, cookies=cookies)
            time.sleep(5)
            
    if pagename_n != "":
        with open("pyBot/wikinews/service/statpages.txt", "w", encoding="utf-8") as f:
            f.write(timestamp + "\n" + pagename_n)
            f.close()
